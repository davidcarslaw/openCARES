...
load(file = "C:/Users/Naomi/My Drive/openCARES/data/rs_dat.rda")
...
|-----------|-------------------------------|
| site_name | Site name                     |
| latitude  | Decimal degrees e.g. 45.45234 |
| longitude | Decimal degrees e.g. 9.19894  |
| altitude  | Elevation (m)                 |
| slope     | Road grade (unit?)            |
...
| co_co2       | mol/mol check these?                                  |
| no_co2       | mol/mol                                  |
| no2_co2      | mol/mol                                  |
| hc_co2       | mol/mol                                  |
| ch4_co2      | mol/mol                                  |
| pm_co2       | mol/mol                                  |
| valid_status | TRUE or FALSE                            |
| speed        | km h^-1^                                 |
| vsp_calc     | vehicle specific power (kW t^-1^)        |

: Table 2. Required measurement data fields.

| Name              | Description                                                                  |
|-------------------|-----------------------------------------------------|
| fuel_type_1       | Fuel type (fuel type 1 for bi-fuel and hybrid) e.g. diesel, petrol, CNG, LPG |
| fuel_type_2       | Fuel type 2 for bi-fuel and hybrid e.g. CNG, LPG, electricity                |
| veh_class         | passenger car, light commercial vehicle, bus, light 2/3/4 wheeler, truck     |
| emission_standard | Euro 5, Euro 6, Euro 6d-TEMP, Euro 6d                                        |
| make_domain       | Manufacturer e.g. Fiat, Nissan, VW                                           |
| reg_date_domestic | Registration date (dd/mm/yyyy)                                               |
| veh_category      | M1, M1G, N1, L1e                                                             |
| mileage           | km                                                                           |
...
# code to be used to calculate g/kg directly from ratios. 
# not matching g/kg columns - check

# rs_dat %>% 
#   mutate(
#     co_gpkg = case_when(
#       fuel_type %in% c("diesel", "petrol", "diesel electricity", "petrol electricity") ~ ((28*co_co2*860)/(1+co_co2+(2*3*hc_co2))*12),
#       fuel_type %in% c("CNG", "petrol CNG", "diesel CNG") ~ ((28*co_co2*750)/(1+co_co2+(3.3*1*hc_co2))*12),
#       fuel_type %in% c("LPG", "diesel LPG", "petrol LPG") ~ ((28*co_co2*820)/(1+co_co2+(1*3*hc_co2))*12),
#       TRUE ~ NA_real_
#     ),
#     no_gpkg = case_when(
#       fuel_type %in% c("diesel", "petrol", "diesel electricity", "petrol electricity") ~ ((30*no_co2*860)/(1+co_co2+(2*3*hc_co2))*12),
#       fuel_type %in% c("CNG", "petrol CNG", "diesel CNG") ~ ((30*no_co2*750)/(1+co_co2+(3.3*1*hc_co2))*12),
#       fuel_type %in% c("LPG", "diesel LPG", "petrol LPG") ~ ((30*no_co2*820)/(1+co_co2+(1*3*hc_co2))*12),
#       TRUE ~ NA_real_
#     ),
#     no2_gpkg = case_when(
#       fuel_type %in% c("diesel", "petrol", "diesel electricity", "petrol electricity") ~ ((46*no2_co2*860)/(1+co_co2+(2*3*hc_co2))*12),
#       fuel_type %in% c("CNG", "petrol CNG", "diesel CNG") ~ ((46*no2_co2*750)/(1+co_co2+(3.3*1*hc_co2))*12),
#       fuel_type %in% c("LPG", "diesel LPG", "petrol LPG") ~ ((46*no2_co2*820)/(1+co_co2+(1*3*hc_co2))*12),
#       TRUE ~ NA_real_
#     ),
#     nox_gpkg = (no_gpkg*(46/30) + no2_gpkg),
#     hc_gpkg = case_when(
#       fuel_type %in% c("diesel", "petrol", "diesel electricity", "petrol electricity") ~ ((2*44*hc_co2*860)/(1+co_co2+(2*3*hc_co2))*12),
#       fuel_type %in% c("CNG", "petrol CNG", "diesel CNG") ~ ((3.33*16*hc_co2*750)/(1+co_co2+(3.3*1*hc_co2))*12),
#       fuel_type %in% c("LPG", "diesel LPG", "petrol LPG") ~ ((1*44*hc_co2*820)/(1+co_co2+(1*3*hc_co2))*12),
#       TRUE ~ NA_real_
#     ),
#     ch4_gpkg = case_when(
#       fuel_type %in% c("diesel", "petrol", "diesel electricity", "petrol electricity") ~ ((14*ch4_co2*860)/(1+co_co2+(2*3*hc_co2))*12),
#       fuel_type %in% c("CNG", "petrol CNG", "diesel CNG") ~ ((14*ch4_co2*750)/(1+co_co2+(3.3*1*hc_co2))*12),
#       fuel_type %in% c("LPG", "diesel LPG", "petrol LPG") ~ ((14*ch4_co2*820)/(1+co_co2+(1*3*hc_co2))*12),
#       TRUE ~ NA_real_
#     ))
```
...
reg_year <- rs_dat %>%
  ...
  pivot_longer(contains("fm"), names_to = "pollutant", values_to = "fm") %>%
  mutate(reg_date_domestic = dmy(reg_date_domestic),
